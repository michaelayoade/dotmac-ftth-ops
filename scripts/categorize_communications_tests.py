#!/usr/bin/env python3
"""
Categorize communications module tests for test pyramid markers.

Analyzes test files to determine if they should be marked as:
- unit: Pure logic tests, no DB/HTTP (validators, utils, template logic)
- integration: Service + DB tests (email service, task service)
- e2e: Router/API tests (email endpoints, metrics endpoints)
"""
    ...
    def visit_ImportFrom(self, node):
        if node.module:
            ...
            if "mock" in node.module.lower() or "unittest.mock" in node.module:
                self.has_mock = True
            if "celery" in node.module or "task" in node.module:
                self.uses_celery = True
...
def analyze_test_file(file_path: Path) -> Dict:
    ...
    confidence = "medium"
    suggested_marker = "integration"  # Default for communications
    reasoning = []

    filename = file_path.name.lower()

    # Strong indicators for unit tests
    if "plugin" in filename and not categorizer.uses_async_session:
        suggested_marker = "unit"
        reasoning.append("Plugin tests without DB are unit tests")
        confidence = "high"

    # Template tests are usually unit
    if "template" in filename and not categorizer.uses_async_session:
        suggested_marker = "unit"
        reasoning.append("Template logic without DB is unit test")
        confidence = "high"
    ...
    # Event listener tests are integration
    if "event" in filename or "listener" in filename:
        suggested_marker = "integration"
        reasoning.append("Event listener tests require event bus")
        confidence = "high"

    # Task service tests are integration
    if "task" in filename and categorizer.uses_celery:
        suggested_marker = "integration"
        reasoning.append("Task service with Celery is integration")
        confidence = "high"
    ...
    # Metrics router tests are E2E
    if "metrics" in filename and "router" in filename:
        suggested_marker = "e2e"
        reasoning.append("Metrics router tests are E2E")
        confidence = "high"
    ...
    return {
        ...
        "dependencies": {
            ...
            "celery": categorizer.uses_celery,
...
def main():
    """Analyze all communications test files."""
    comm_tests_dir = Path("tests/communications")
    test_files = sorted(comm_tests_dir.glob("test_*.py"))

    results = []
    summary = {"unit": [], "integration": [], "e2e": [], "unknown": []}

    print("=" * 80)
    print("COMMUNICATIONS MODULE TEST CATEGORIZATION")
    print("=" * 80)
    ...
    if summary["unit"]:
        unit_files = [f"tests/communications/{f}" for f in summary["unit"]]
    ...
    if summary["integration"]:
        int_files = [f"tests/communications/{f}" for f in summary["integration"]]
    ...
    if summary["e2e"]:
        e2e_files = [f"tests/communications/{f}" for f in summary["e2e"]]
        print(f"\n# Mark {len(summary['e2e'])} E2E tests:")
        print(f"python scripts/batch_add_markers.py --marker e2e {' '.join(e2e_files)}")

    # Save detailed results to JSON
    output_file = "communications_test_categorization.json"
