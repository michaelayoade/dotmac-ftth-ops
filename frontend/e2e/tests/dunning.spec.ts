...
test.describe('Dunning & Collections Management', () => {
  ...
  test.describe('Creating Campaigns', () => {
    ...
    test('should validate required fields', async ({ page }) => {
      ...
      await expect(
        page.locator('text=/name is required|required field/i')
      ).toBeVisible();
    });
  ...
  test.describe('Updating Campaigns', () => {
    test('should update campaign details', async ({ page }) => {
      ...
      await campaignRows.first().locator(
        'button:has-text("Edit"), button:has-text("Manage"), [data-testid="edit-button"]'
      ).click();
  ...
  test.describe('Pausing and Resuming Campaigns', () => {
    test('should pause an active campaign', async ({ page }) => {
      const campaignRows = page.locator('[data-testid="campaign-row"]:has-text("active"), tbody tr:has-text("active")');
      const count = await campaignRows.count();
      ...
      // Click pause button
      await firstActiveCampaign.locator(
        'button:has-text("Pause"), [data-testid="pause-button"]'
      ).click();
      ...
      await expect(
        page.locator('text=/paused|campaign paused/i')
      ).toBeVisible();
    });

    test('should resume a paused campaign', async ({ page }) => {
      const pausedCampaigns = page.locator('[data-testid="campaign-row"]:has-text("paused"), tbody tr:has-text("paused")');
      ...
      // Click resume button
      await firstPausedCampaign.locator(
      ...
      await expect(
        page.locator('text=/active|campaign resumed/i')
      ).toBeVisible();
    });
  });
  ...
  test.describe('Starting Executions', () => {
    test('should start a new execution', async ({ page }) => {
      ...
      // Verify success
      await expect(
        page.locator('text=/execution started|success/i')
      ).toBeVisible();
    });
  });
  ...
  test.describe('Canceling Executions', () => {
    test('should cancel an active execution', async ({ page }) => {
      ...
      await expect(
        page.locator('text=/cancelled|execution cancelled/i')
      ).toBeVisible();
    });
  });
  ...
  test.describe('Statistics and Analytics', () => {
    test('should display dunning statistics', async ({ page }) => {
      ...
      if (await statsSection.isVisible()) {
        ...
        await expect(statsSection.locator('text=/total campaigns|active campaigns/i')).toBeVisible();
        await expect(statsSection.locator('text=/total executions|active executions/i')).toBeVisible();
  ...
  test.describe('Error Handling', () => {
    test('should handle network errors gracefully', async ({ page, context }) => {
      ...
      await context.route('**/api/v1/billing/dunning/campaigns', (route) => {
        route.abort('failed');
      });
    ...
    test('should handle invalid campaign configuration', async ({ page }) => {
      ...
      await expect(
        page.locator('text=/invalid|must be positive|greater than 0/i')
      ).toBeVisible();
    });
    ...
    test('should handle campaign with active executions deletion', async ({ page }) => {
      ...
      const campaignWithExecutions = page.locator(
        '[data-testid="campaign-row"]:has-text("active")'
      ).first();
      ...
      if (await campaignWithExecutions.isVisible()) {
        ...
        const warningMessage = page.locator(
          'text=/active executions|cannot delete|in use/i'
        );
