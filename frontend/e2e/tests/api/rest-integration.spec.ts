...
test.describe("REST API Integration", () => {
  ...
  async function authenticate(request: APIRequestContext): Promise<string> {
    const response = await request.post(`${BASE_URL}/api/isp/v1/auth/login`, {
      data: {
  ...
  test.describe("API Error Handling", () => {
    test("should handle 422 validation errors in UI", async ({ page }) => {
      ...
      await page.route("**/api/isp/v1/**", (route) => {
    ...
    test("should handle network timeouts", async ({ page }) => {
      ...
      await page.route("**/api/isp/v1/**", async (route) => {
  ...
  test.describe("API Authentication", () => {
    ...
    test("should include auth headers in API requests", async ({ page }) => {
      ...
      page.on("request", (request) => {
        if (request.url().includes("/api/isp/v1/")) {
  ...
  test.describe("CORS and Security", () => {
    test("should handle CORS properly", async ({ request }) => {
      ...
      const response = await request.fetch(`${BASE_URL}/api/isp/v1/health`, {
        method: "OPTIONS",
        headers: {
          Origin: APP_URL,
  ...
  test.describe("API Performance", () => {
    test("should handle concurrent requests", async ({ page }) => {
      ...
      page.on("response", (response) => {
        if (response.url().includes("/api/isp/v1/")) {
          currentConcurrent--;
        }
      });
