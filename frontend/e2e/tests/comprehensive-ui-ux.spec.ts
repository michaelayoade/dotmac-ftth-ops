  ...
  test.describe("Page Load & Visual Design", () => {
    ...
    test("customer portal displays correctly", async ({ page }) => {
      ...
      await page.screenshot({
        path: "test-results/screenshots/customer-portal.png",
        fullPage: true,
      });
    });
  });
  ...
  test.describe("Navigation & User Flow", () => {
    ...
    test("breadcrumbs work correctly", async ({ page }) => {
      ...
      const breadcrumb = page
        .locator("[aria-label*='breadcrumb'], [data-testid*='breadcrumb'], nav ol, nav ul")
        .first();
  ...
  test.describe("Accessibility", () => {
    ...
    test("forms have proper labels", async ({ page }) => {
      ...
      for (let i = 0; i < inputCount; i++) {
        ...
        console.log(
          `  Input ${i + 1}: id="${id}" name="${name}" aria-label="${ariaLabel}" placeholder="${placeholder}"`,
        );
      }
    });
  });
