...
test.describe('Customer Portal', () => {
  ...
  test.describe('Profile Management', () => {
    ...
    test('should display service address', async ({ page }) => {
      ...
      const addressSection = page.locator('[data-testid="service-address"], section:has-text("Service Address")');
    ...
    test('should upgrade service plan', async ({ page }) => {
      ...
      if (await upgradeButton.isVisible()) {
        ...
        if (count > 1) {
          ...
          // Verify success
          await expect(
            page.locator('text=/upgraded|success/i')
  ...
  test.describe('Invoices and Billing', () => {
    ...
    test('should download invoice PDF', async ({ page }) => {
      ...
      const downloadButtons = page.locator('button:has-text("Download"), [data-testid="download-invoice"]');
  ...
  test.describe('Payment Processing', () => {
    ...
    test('should set default payment method', async ({ page }) => {
      ...
      if (count > 1) {
        ...
        await expect(
          paymentMethods.nth(1).locator('text=/default/i')
        ).toBeVisible();
      }
    });
    ...
    test('should make a payment', async ({ page }) => {
      ...
      const unpaidInvoices = page.locator('[data-testid="invoice-row"]:has-text("unpaid"), tr:has-text("due")');
      const count = await unpaidInvoices.count();

      if (count > 0) {
        // Click pay button on first unpaid invoice
        await unpaidInvoices.first().locator('button:has-text("Pay"), button:has-text("Pay Now")').click();

        // Select payment method
        const paymentMethodSelect = page.locator('[name="payment_method"], [data-testid="payment-method-select"]');
        ...
        // Verify success
        await expect(
          page.locator('text=/payment successful|paid/i')
        ).toBeVisible({ timeout: 15000 });
      }
    });
    ...
    test('should enable auto-pay', async ({ page }) => {
      ...
      if (count > 0) {
        ...
        const autoPayToggle = paymentMethods.first().locator('[role="switch"], input[type="checkbox"]');
  ...
  test.describe('Usage Monitoring', () => {
    ...
    test('should display usage limit warning', async ({ page }) => {
      ...
      const warningMessage = page.locator('[data-testid="usage-warning"], [class*="alert"]:has-text("usage")');
  ...
  test.describe('Support Tickets', () => {
    ...
    test('should create support ticket', async ({ page }) => {
      ...
      // Verify success
      await expect(
        page.locator('text=/ticket created|success/i')
      ).toBeVisible({ timeout: 10000 });
