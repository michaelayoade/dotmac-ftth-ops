...
<nav className="fixed top-0 left-0 right-0 z-50 bg-card border-b border-border">
  <div className="flex h-16 items-center justify-between px-4 sm:px-6 lg:px-8">
    ...
    <div className="flex items-center">
      ...
      <div className="ml-4 lg:ml-0">{/* Branding logo or text */}</div>
    </div>
...
<div ref={userMenuRef} className="relative">
  <button onClick={() => setUserMenuOpen(!userMenuOpen)}>{/* Button */}</button>
  {userMenuOpen && <div className="absolute right-0 mt-2">{/* Menu */}</div>}
</div>;
```
...
<div className="fixed inset-y-0 left-0 w-64 flex flex-col">
  ...
  <div className="flex-1 overflow-y-auto">
    {sections.map((section) => (
      <div key={section.id}>
        <SectionHeader />
        {isExpanded && (
          <div className="max-h-64 overflow-y-auto">{/* Section items scroll separately */}</div>
        )}
      </div>
    ))}
  </div>
  ...
  {/* Bottom - Fixed */}
  <div className="flex-shrink-0">{/* Version info */}</div>
</div>
```
...
useEffect(() => {
  const currentPage = sections.flatMap((s) => s.items || []).find((item) => pathname === item.href);
  ...
  return visibleSections
    .map((section) => ({
      ...
      items: section.items?.filter(
        (item) =>
          item.name.toLowerCase().includes(query) || section.label.toLowerCase().includes(query),
      ),
    }))
    .filter(
      (section) =>
        section.label.toLowerCase().includes(query) || (section.items && section.items.length > 0),
    );
}, [navSearch, visibleSections]);
...
<div ref={sidebarRef} className="sidebar">
  <button data-autofocus className="lg:hidden" onClick={() => setSidebarOpen(false)}>
    <X />
  </button>
