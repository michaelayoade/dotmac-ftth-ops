...
function DevicesPageContent() {
  ...
  const { data: devices = [], isLoading } = useQuery<Device[]>({
    ...
    queryFn: async () => {
      ...
      const response = await fetch(`${apiBaseUrl}/api/isp/v1/genieacs/devices?${params.toString()}`, {
        credentials: "include",
  ...
  const { data: stats } = useQuery<DeviceStats>({
    ...
    queryFn: async () => {
      const response = await fetch(`${apiBaseUrl}/api/isp/v1/genieacs/devices/stats`, {
        credentials: "include",
        headers: { "Content-Type": "application/json" },
      });
