...
function FirmwarePageContent() {
  ...
  const { data: availableVersions = [] } = useQuery<FirmwareVersion[]>({
    ...
    queryFn: async () => {
      const response = await fetch(
        `${apiBaseUrl}/api/isp/v1/genieacs/firmware/versions?model=${device?.summary?.model}`,
        { credentials: "include" },
      );
  ...
  const { data: upgradeJob } = useQuery<UpgradeJob | null>({
    ...
    queryFn: async () => {
      const response = await fetch(
        `${apiBaseUrl}/api/isp/v1/genieacs/devices/${deviceId}/firmware/status`,
        { credentials: "include" },
      );
  ...
  const upgradeMutation = useMutation({
    mutationFn: async (version: string) => {
      const response = await fetch(
        `${apiBaseUrl}/api/isp/v1/genieacs/devices/${deviceId}/firmware/upgrade`,
        {
          method: "POST",
          credentials: "include",
