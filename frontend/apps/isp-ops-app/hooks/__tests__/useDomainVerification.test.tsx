/**
 * Jest Mock Tests for useDomainVerification hook
 * Tests domain verification system with Jest mocks instead of MSW
 */
...
jest.mock("@/lib/services/domain-verification-service", () => ({
  domainVerificationService: {
    ...
    getStatus: jest.fn(),
    validateDomain: jest.fn((domain: string) => {
...
const mockedService = domainVerificationService as jest.Mocked<typeof domainVerificationService>;
...
describe("useDomainVerification", () => {
  ...
  function createMockVerificationResponse(
    ...
    return {
      ...
      instructions: {
        ...
        dns_record: {
          type: "TXT",
          name: "_dotmac-verification",
          value: "verification-token-123",
        },
        steps: ["Add DNS TXT record", "Wait for propagation", "Click verify"],
      },
      verified_at: null,
      ...overrides,
    };
  ...
  describe("Mutation Hooks", () => {
    describe("initiate verification", () => {
      it("should initiate DNS TXT verification successfully", async () => {
        const mockResponse = createMockVerificationResponse({
          domain: "example.com",
          method: "dns_txt",
          status: "pending",
        });

        mockedService.initiateVerification.mockResolvedValueOnce(mockResponse);
      ...
      it("should initiate meta tag verification successfully", async () => {
        const mockResponse = createMockVerificationResponse({
          ...
          instructions: {
            type: "HTML Meta Tag",
            meta_tag: '<meta name="dotmac-verification" content="token-123" />',
            steps: [],
          },
        });
      ...
      it("should initiate file upload verification successfully", async () => {
        const mockResponse = createMockVerificationResponse({
          ...
          instructions: {
            ...
            file_content: "verification-token-123",
            steps: [],
          },
        });
    ...
    describe("check verification", () => {
      it("should verify domain successfully with correct token", async () => {
        const initiateResponse = createMockVerificationResponse({
          token: "correct-token-123",
        });

        const checkResponse = createMockVerificationResponse({
          status: "verified",
          verified_at: new Date().toISOString(),
        });

        mockedService.initiateVerification.mockResolvedValueOnce(initiateResponse);
        mockedService.checkVerification.mockResolvedValueOnce(checkResponse);
    ...
    describe("remove domain", () => {
      it("should remove domain successfully", async () => {
        const removeResponse: DomainRemovalResponse = {
          domain: "example.com",
          status: "removed",
  ...
  describe("Cache Invalidation", () => {
    it("should invalidate domain status query after successful initiate", async () => {
      const initiateResponse = createMockVerificationResponse();
      const statusResponse = createMockStatusResponse({
        domain: "example.com",
        is_verified: false,
        status: "pending",
      });
      ...
      mockedService.getStatus.mockResolvedValueOnce(statusResponse);
    ...
    it("should invalidate domain status query after successful check", async () => {
      ...
      mockedService.getStatus
        .mockResolvedValueOnce(createMockStatusResponse())
    ...
    it("should invalidate domain status query after successful remove", async () => {
      ...
      mockedService.getStatus.mockResolvedValueOnce(
...
describe("useDomainStatus", () => {
  ...
  describe("Query Hook", () => {
    it("should fetch domain status successfully", async () => {
      ...
      mockedService.getStatus.mockResolvedValueOnce(mockStatus);
...
describe("Real-world scenarios", () => {
  ...
  it("should handle verification failure and retry", async () => {
    ...
    const failedCheckResponse = createMockVerificationResponse({
      domain: "retry.example.com",
      status: "failed",
      error_message: "Token verification failed",
    });

    const successCheckResponse = createMockVerificationResponse({
      domain: "retry.example.com",
      status: "verified",
      verified_at: new Date().toISOString(),
    });
    ...
    mockedService.initiateVerification.mockResolvedValueOnce(initiateResponse);
    mockedService.checkVerification
      .mockResolvedValueOnce(failedCheckResponse)
      .mockResolvedValueOnce(successCheckResponse);
