 ...
 *
 * Handles WebSocket connections to /api/isp/v1/realtime/ws endpoints with:
...
export class WebSocketEndpoints {
  ...
  sessions(config?: Partial<WebSocketConfig>): WebSocketClient {
    return createWebSocketClient({
      endpoint: `${this.baseUrl}/api/isp/v1/realtime/ws/sessions`,
      ...config,
    });
  }
  ...
  job(jobId: string, config?: Partial<WebSocketConfig>): WebSocketClient {
    return createWebSocketClient({
      endpoint: `${this.baseUrl}/api/isp/v1/realtime/ws/jobs/${jobId}`,
      ...config,
    });
  }
  ...
  campaign(campaignId: string, config?: Partial<WebSocketConfig>): WebSocketClient {
    return createWebSocketClient({
      endpoint: `${this.baseUrl}/api/isp/v1/realtime/ws/campaigns/${campaignId}`,
      ...config,
    });
  }
}
