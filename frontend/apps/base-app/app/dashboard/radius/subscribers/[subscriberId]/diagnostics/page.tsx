...
function DiagnosticsPageContent() {
  ...
  const useCheckMutation = (endpoint: string, checkName: string) =>
    useMutation({
      mutationFn: async () => {
  ...
  const connectivityCheck = useCheckMutation("connectivity", "Connectivity Check");
  const radiusSessionCheck = useCheckMutation("radius-sessions", "RADIUS Session Check");
  const onuStatusCheck = useCheckMutation("onu-status", "ONU Status Check");
  const cpeStatusCheck = useCheckMutation("cpe-status", "CPE Status Check");
  const ipVerificationCheck = useCheckMutation("ip-verification", "IP Verification");
  const cpeRestartCheck = useCheckMutation("restart-cpe", "CPE Restart");
