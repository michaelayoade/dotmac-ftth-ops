...
test.describe("Global Search UI", () => {
  ...
  test("should sync search parameters with URL", async ({ page }) => {
    ...
    await expect(page.locator('input[placeholder*="Search for anything"]')).toHaveValue("url test");
  });
});
...
test.describe("Global Search - Error Handling", () => {
  ...
  test("should handle network timeout", async ({ page }) => {
    ...
    await expect(page.locator("text=/Error|Timeout|Failed/i").first()).toBeVisible({
      timeout: 15000,
    });
  });
});
