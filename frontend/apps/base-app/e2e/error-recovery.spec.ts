...
test.describe("Session Expiry Handling", () => {
  test("should detect expired token and redirect to login", async ({ page }) => {
    ...
    await expect(page.locator("text=/session.*expired|logged out|sign in again/i")).toBeVisible({
      timeout: 5000,
    });
  });
...
test.describe("Network Error Recovery", () => {
  test("should show offline indicator when network is unavailable", async ({ page }) => {
    ...
    await expect(page.locator("text=/offline|no connection|check.*connection/i")).toBeVisible({
      timeout: 5000,
    });
  });
...
test.describe("Server Error Handling", () => {
  ...
  test("should handle 403 forbidden errors", async ({ page }) => {
    ...
    await expect(page.locator("text=/permission|access denied|not authorized/i")).toBeVisible({
      timeout: 5000,
    });
  });
  ...
  test("should handle 429 rate limit errors", async ({ page }) => {
    ...
    await expect(page.locator("text=/too many requests|rate limit|slow down/i")).toBeVisible({
      timeout: 5000,
    });
