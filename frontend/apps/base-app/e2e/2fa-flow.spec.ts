...
test.describe("2FA Setup Flow", () => {
  ...
  test("should navigate to 2FA setup from settings", async ({ page }) => {
    ...
    if ((await setupButton.count()) > 0) {
      ...
      await expect(page.locator("text=/Set up|Configure|Enable.*2FA/i")).toBeVisible({
        timeout: 5000,
      });
    }
  });
  ...
  test("should verify TOTP code during setup", async ({ page }) => {
    ...
    await expect(page.locator("text=/enabled|activated|success/i")).toBeVisible({ timeout: 5000 });
  });
  ...
  test("should save backup codes during setup", async ({ page }) => {
    ...
    expect((await downloadButton.count()) + (await copyButton.count())).toBeGreaterThan(0);
  });
});
...
test.describe("2FA Login Flow", () => {
  test("should show 2FA prompt after password entry", async ({ page }) => {
    ...
    await expect(page.locator("text=/Enter.*code|Verification Code|2FA Code/i")).toBeVisible({
      timeout: 5000,
    });
