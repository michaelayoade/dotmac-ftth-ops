...
export function CustomerBilling({ customerId }: CustomerBillingProps) {
  ...
  const fetchBillingData = useCallback(async () => {
    try {
      ...
      const [invoicesRes, paymentsRes, summaryRes] = await Promise.all([
        apiClient.get<{ invoices: Invoice[] }>(`/api/v1/customers/${customerId}/invoices`),
        apiClient.get<{ payments: Payment[] }>(`/api/v1/customers/${customerId}/payments`),
        apiClient.get<BillingSummary>(`/api/v1/customers/${customerId}/billing-summary`),
      ]);
