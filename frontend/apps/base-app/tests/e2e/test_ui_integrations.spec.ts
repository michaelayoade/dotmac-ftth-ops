...
describe("E2E: Global Search UI Integration", () => {
  ...
  test("should handle search errors gracefully", async () => {
    ...
    await expect(searchService.search({ q: "test", limit: 10, page: 1 })).rejects.toThrow(
  ...
  test("should reindex entity", async () => {
    ...
    await expect(searchService.reindex({ entity_type: "customer" })).resolves.not.toThrow();
...
describe("E2E: Error Handling and Edge Cases", () => {
  ...
  test("should handle unauthorized access (401)", async () => {
    ...
    await expect(auditService.listActivities({})).rejects.toThrow("Authentication required");
  });
  ...
  test("should handle rate limiting (429)", async () => {
    ...
    await expect(communicationsService.getStatistics({})).rejects.toThrow("Rate limit exceeded");
  });
});
