...
import { generateTestUser, generateTestServer, generateTestPeer, generateMultipleServers } from '../../fixtures/test-data';
import {
  createTestUser,
  loginUser,
  createServer,
...
test.describe('WireGuard Dashboard', () => {
  ...
  test.describe('Server List Overview', () => {
    ...
    test('should display peer count for each server', async ({ page }) => {
      ...
      // Assert
      await expect(page.locator(`[data-server-id="${created.id}"] [data-testid="peer-count"]`)).toContainText('5');
    });
  ...
  test.describe('Charts and Graphs', () => {
    ...
    test('should display peer activity chart', async ({ page }) => {
      ...
      for (let i = 0; i < 5; i++) {
        const peer = generateTestPeer(created.id, { status: i < 3 ? 'active' : 'inactive' });
        await createPeer(peer, authToken);
      }
  ...
  test.describe('Alerts and Notifications', () => {
    test('should show alert for degraded servers', async ({ page }) => {
      // Arrange
      const server = generateTestServer({ status: 'degraded', name: 'degraded-server' });
      await createServer(server, authToken);
