...
test.describe("Real-Time WebSocket Connections", () => {
  ...
  test.describe("Connection Establishment", () => {
    ...
    test("should establish multiple WebSocket connections", async ({ page }) => {
      ...
      const wsCampaignsStream = page.locator(
        '[data-stream="campaigns-ws"][data-status="connected"]',
      );
  ...
  test.describe("Bi-directional Communication", () => {
    ...
    test("should handle command responses", async ({ page }) => {
      ...
      await expect(page.locator("text=Job resumed successfully")).toBeVisible({
        timeout: 10000,
      });
    });
    ...
    test("should respond to server pong messages", async ({ page }) => {
      ...
      await expect(
        page.locator('[data-testid="ws-status"][data-status="connected"]'),
      ).toBeVisible();
    });
  });
  ...
  test.describe("Job Control Commands", () => {
    test("should send pause job command", async ({ page }) => {
      ...
      // Assert
      await expect(page.locator('[data-job-id="running-job"][data-status="paused"]')).toBeVisible({
        timeout: 10000,
      });
    });
    ...
    test("should send resume job command", async ({ page }) => {
      ...
      // Assert
      await expect(page.locator('[data-job-id="paused-job"][data-status="running"]')).toBeVisible({
        timeout: 10000,
      });
    });
    ...
    test("should send cancel job command", async ({ page }) => {
      ...
      // Assert
      await expect(
        page.locator('[data-job-id="running-job"][data-status="cancelled"]'),
    ...
    test("should show error if command fails", async ({ page }) => {
      ...
      // Assert
      await expect(page.locator("text=Failed to pause job")).toBeVisible({
        timeout: 10000,
      });
    });
  });
  ...
  test.describe("Campaign Control Commands", () => {
    test("should send pause campaign command", async ({ page }) => {
      ...
      // Assert
      await expect(
        page.locator('[data-campaign-id="active-campaign"][data-status="paused"]'),
      ).toBeVisible({ timeout: 10000 });
    });
    ...
    test("should send resume campaign command", async ({ page }) => {
      ...
      // Assert
      await expect(
        page.locator('[data-campaign-id="paused-campaign"][data-status="running"]'),
      ).toBeVisible({ timeout: 10000 });
    });
    ...
    test("should show real-time campaign progress", async ({ page }) => {
      ...
      const initialProgress = await page
        .locator('[data-campaign-id="active-campaign"] [data-testid="progress"]')
        .textContent();
  ...
  test.describe("Connection Health", () => {
    test("should detect connection timeout", async ({ page }) => {
      ...
      // Assert
      await expect(
        page.locator('[data-testid="ws-status"][data-status="reconnecting"]'),
      ).toBeVisible();
    });
