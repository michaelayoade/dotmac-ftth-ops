...
describe("useCreditNotes", () => {
  ...
  it("should fetch credit notes successfully", async () => {
    ...
    expect(result.current.data?.[0]).toEqual({
      ...
      status: "issued",
      downloadUrl: "/api/isp/v1/billing/credit-notes/cn-1/download",
    });

    expect(global.fetch).toHaveBeenCalledWith(
      "http://localhost:8000/api/isp/v1/billing/credit-notes?limit=5",
  ...
  it("should generate correct download URL", async () => {
    ...
    expect(result.current.data?.[0]?.downloadUrl).toBe(
      "/api/isp/v1/billing/credit-notes/cn-123/download",
    );
  });
