...
export function useLogs(filters: LogsFilter = {}) {
    ...
    queryFn: async () => {
      try {
        ...
        const response = await axios.get<LogsResponse>(
          `${apiBaseUrl}/api/platform/v1/monitoring/logs?${params.toString()}`,
  ...
  const statsQuery = useQuery({
    ...
    queryFn: async () => {
      try {
        const response = await axios.get<LogStats>(`${apiBaseUrl}/api/platform/v1/monitoring/logs/stats`, {
          withCredentials: true,
        });
        return response.data;
      } catch (err: unknown) {
    ...
    queryFn: async () => {
      try {
        const response = await axios.get<string[]>(
          `${apiBaseUrl}/api/platform/v1/monitoring/logs/services`,
          { withCredentials: true },
        );
        return response.data;
      } catch (err: unknown) {
