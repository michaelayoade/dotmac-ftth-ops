...
import { resellerAppConfig } from "@/lib/config";
import { getResellerToken } from "@/lib/auth/token-utils";
...
async function fetchPartnerDashboard(): Promise<PartnerDashboardStats> {
  const response = await fetch(
    resellerAppConfig.api.buildUrl("/partners/portal/dashboard"),
    {
...
async function fetchPartnerProfile(): Promise<PartnerProfile> {
  const response = await fetch(
    resellerAppConfig.api.buildUrl("/partners/portal/profile"),
    {
      headers: getAuthHeaders(),
    }
  );
...
async function fetchPartnerReferrals(
  ...
  const url = resellerAppConfig.api.buildUrl(
...
async function submitReferral(data: {
  ...
  const response = await fetch(
    resellerAppConfig.api.buildUrl("/partners/portal/referrals"),
    {
      method: "POST",
      headers: getAuthHeaders(),
      body: JSON.stringify(data),
...
export function usePartnerProfile() {
  return useQuery({
    queryKey: ["reseller-portal-profile"],
...
export function useUpdatePartnerProfile() {
  ...
  return useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["reseller-portal-profile"] });
      queryClient.invalidateQueries({ queryKey: ["reseller-portal-dashboard"] });
    },
  });
}
...
export function usePartnerReferrals(limit?: number, offset?: number) {
  return useQuery({
    queryKey: ["reseller-portal-referrals", limit, offset],
...
export function useSubmitReferral() {
  ...
  return useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["reseller-portal-referrals"] });
      queryClient.invalidateQueries({ queryKey: ["reseller-portal-dashboard"] });
    },
  });
}
...
export function usePartnerCommissions(limit?: number, offset?: number) {
  return useQuery({
    queryKey: ["reseller-portal-commissions", limit, offset],
...
export function usePartnerCustomers(limit?: number, offset?: number) {
  return useQuery({
    queryKey: ["reseller-portal-customers", limit, offset],
...
export function usePartnerStatements(limit?: number, offset?: number) {
  return useQuery({
    queryKey: ["reseller-portal-statements", limit, offset],
...
export function usePartnerPayoutHistory(limit?: number, offset?: number) {
  return useQuery({
    queryKey: ["reseller-portal-payouts", limit, offset],
