/**
 * Better Auth configuration for CLI migrations (ESM).
 * Mirrors frontend/shared/lib/better-auth/auth.ts without TS/ESM interop issues.
 */

import { betterAuth } from "/tmp/better-auth-cli/node_modules/better-auth/dist/index.mjs";
import { organization } from "/tmp/better-auth-cli/node_modules/better-auth/dist/plugins/organization/index.mjs";
import { createAccessControl } from "/tmp/better-auth-cli/node_modules/better-auth/dist/plugins/access/index.mjs";
import { Pool } from "pg";
...
const createAuthInstance = () => {
  ...
  pool
    .query("select 1")
    .catch((err) => {
      console.error("[better-auth-config] pool connectivity failed:", err);
      throw err;
    });

  return betterAuth({
    database: { provider: "pg", connection: pool },
    baseURL: authUrl,
    secret: authSecret,
    user: {
      additionalFields: {
    ...
    plugins: [
      organization({
        enabled: true,
        organization: { roles: ["member", "admin", "owner", "billing"], defaultRole: "member" },
        member: {
          additionalFields: {
...
try {
  ...
  console.error("[better-auth-config] failed to init auth:", err);
  throw err;
}
export default auth;
export { auth };
