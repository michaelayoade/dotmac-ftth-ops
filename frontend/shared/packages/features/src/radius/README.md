...
- `PUT /api/v1/radius/bandwidth-profiles/{id}` - Update profile
...
- `PUT /api/v1/radius/nas-devices/{id}` - Update device
- `DELETE /api/v1/radius/nas-devices/{id}` - Delete device
- `POST /api/v1/radius/nas-devices/{id}/test` - Test connection
...
const handleDisconnect = async (sessionId: string) => {
  try {
    await apiClient.post(`/api/v1/radius/sessions/${sessionId}/disconnect`);
    toast({
      title: "Success",
...
const changeBandwidth = async (sessionId: string, profileId: string) => {
  try {
    await apiClient.post(`/api/v1/radius/sessions/${sessionId}/coa`, {
