...
export const asyncValidationRules = {
    ...
    validate: async (value: unknown) => {
      ...
      try {
        const response = await apiClient.get(
          `/api/isp/v1/auth/check-email?email=${encodeURIComponent(value)}`,
        );
  ...
  uniqueUsername: (apiClient: any): AsyncValidationRule => ({
    validate: async (value: unknown) => {
      ...
      try {
        const response = await apiClient.get(
          `/api/isp/v1/auth/check-username?username=${encodeURIComponent(value)}`,
        );
  ...
  validAddress: (apiClient: any): AsyncValidationRule => ({
    validate: async (value: unknown) => {
      ...
      try {
        const response = await apiClient.post("/api/isp/v1/geocoding/validate", {
          address: value,
        });
  ...
  validPhoneNumber: (apiClient: any): AsyncValidationRule => ({
    validate: async (value: unknown) => {
      ...
      try {
        const response = await apiClient.post("/api/isp/v1/validation/phone", {
          phone: value,
        });
        return response.data.valid;
      } catch (error) {
  ...
  validCreditCard: (apiClient: any): AsyncValidationRule => ({
    validate: async (value: unknown) => {
      ...
      try {
        const response = await apiClient.post("/api/isp/v1/payment/validate-card", {
          card_number: cleaned,
        });
        return response.data.valid;
      } catch (error) {
  ...
  availableSubdomain: (apiClient: any): AsyncValidationRule => ({
    validate: async (value: unknown) => {
      ...
      try {
        const response = await apiClient.get(
          `/api/platform/v1/tenants/check-subdomain?subdomain=${encodeURIComponent(value)}`,
        );
  ...
  validIBAN: (apiClient: any): AsyncValidationRule => ({
    validate: async (value: unknown) => {
      ...
      try {
        const response = await apiClient.post("/api/isp/v1/validation/iban", {
          iban: value,
        });
        return response.data.valid;
      } catch (error) {
